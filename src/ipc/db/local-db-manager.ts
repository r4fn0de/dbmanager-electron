...
function normalizeEmbeddedPostgresCtor(value: unknown): EmbeddedPostgresCtor {
  if (typeof value === "function") {
    return value as EmbeddedPostgresCtor;
  }
...
function loadEmbeddedPostgres(): EmbeddedPostgresCtor {
  ...
  if (embeddedPostgresRuntimePath) {
    ...
    if (!existsSync(runtimePkgJson)) {
      throw new Error(
        `Embedded Postgres runtime package.json not found at: ${runtimePkgJson}`
      );
    }
  ...
  if (embeddedPostgresCached) {
    return embeddedPostgresCached;
  }
  ...
  const candidates = [
    ...
    base
      ? join(base, "app.asar.unpacked", "node_modules", "embedded-postgres")
      : null,
...
const _SQLITE_SUBDIR = "local-dbs/sqlite";
...
function _getEmbeddedPostgresVersionFromDeps(): string {
  try {
...
function downloadFileSync(url: string, destination: string): void {
  ...
  if (result.status !== 0) {
    throw new Error(
      `Failed to download ${url}: ${result.stderr.toString() || result.stdout.toString()}`
    );
  }
}
...
function extractTarGzSync(archivePath: string, outputDir: string): void {
  ...
  if (result.status !== 0) {
    throw new Error(
      `Failed to extract ${archivePath}: ${result.stderr.toString() || result.stdout.toString()}`
    );
  }
}

function _installNpmTarballSync(
  packageName: string,
  version: string,
  ...
  const tempDir = join(
    tmpdir(),
    `tarsdb-${Date.now()}-${Math.random().toString(16).slice(2)}`
  );
...
function ensureEmbeddedPostgresRuntimePackages(): void {
  ...
  for (const pkg of packages) {
    ...
    if (existsSync(join(targetPkgPath, "package.json"))) {
      continue;
    }
  ...
  if (
    !(
      existsSync(embeddedPkgPath) &&
      existsSync(platformPkgPath) &&
      existsSync(pgPkgPath)
    )
  ) {
    throw new Error(
      "Embedded Postgres runtime is incomplete (missing embedded-postgres/platform/pg packages)."
    );
  }
...
export class LocalDbManager {
  private readonly runningInstances: Map<string, RunningInstance> = new Map();
  private readonly sqliteHandles: Map<string, SqliteHandle> = new Map();
  private metaCache: LocalDbMeta[] | null = null;
  /** Branch metadata cache, keyed by local DB instance ID. */
  private readonly branchCache: Map<string, BranchMeta[]> = new Map();
  /** Per-local-db operation lock to avoid concurrent branch mutations. */
  private readonly localDbLocks: Map<string, Promise<void>> = new Map();
  ...
  private normalizeMetaList(list: LocalDbMeta[]): LocalDbMeta[] {
    ...
    for (const meta of list) {
      ...
      if (engine === "sqlite") {
        ...
        const filePath =
          meta.file_path ?? join(getBaseDataDir(), "sqlite", `${meta.id}.db`);
  ...
  private decryptMetaSecrets(list: LocalDbMeta[]): {
    list: LocalDbMeta[];
    changed: boolean;
  } {
    let changed = false;
  ...
  private async loadMetaList(): Promise<LocalDbMeta[]> {
    ...
    try {
      ...
      const { list: decrypted, changed: decryptedChanged } =
        this.decryptMetaSecrets(parsed);
  ...
  private async saveBranchList(
    ...
    await writeFile(
      this.getBranchesPath(localDbId),
  ...
  private buildBranchConnectionString(
    meta: LocalDbMeta,
    branchDbName: string
  ): string {
  ...
  private async withLocalDbLock<T>(
    localDbId: string,
    operation: () => Promise<T>
  ): Promise<T> {
  ...
  private async listPhysicalDatabases(
    localDbId: string
    ...
    try {
      ...
      try {
        ...
        const rows = (
          result as { rows?: Array<{ datname?: string }> } | undefined
        )?.rows;
      ...
      console.warn(
        `[local-db] Skipping branch metadata reconciliation for ${localDbId}:`,
        err
      );
      return null;
    }
  ...
  private async reconcileMissingBranchDatabases(
    localDbId: string,
    meta: LocalDbMeta
  ): Promise<BranchMeta[]> {
    ...
    const missing = branches.filter(
      (b) => !(b.isMain || physical.has(b.dbName))
    );
    if (missing.length === 0) {
      return branches;
    }
  ...
  private metaToInfo(meta: LocalDbMeta, running: boolean): LocalDbInfo {
    ...
    const filePath =
      meta.file_path ??
      (isSqlite
        ? join(getBaseDataDir(), "sqlite", `${meta.id}.db`)
        : undefined);
    ...
    if (!isSqlite) {
      ...
      if (activeBranch) {
        databaseName = activeBranch.dbName;
      }
    }
    ...
    return {
      ...
      postgres_version:
        meta.engine === "postgresql" ? meta.postgres_version : undefined,
      running,
  ...
  async findAvailablePort(startPort = 5432, maxAttempts = 20): Promise<number> {
    const usedPorts = new Set((await this.loadMetaList()).map((m) => m.port));
  ...
  private tryHydrateSymlinks(pkgPath: string): void {
    ...
    if (result.error) {
      console.warn(
        "[local-db] Could not run hydrate-symlinks script:",
      ...
      if (existsSync(libDir)) {
        ...
        if (existsSync(symlinksConfig)) {
          try {
            ...
            const config = runtimeRequire(symlinksConfig) as Array<{
              target: string;
            }>;
  ...
  private async createPostgres(input: {
    ...
    this.runningInstances.set(id, { meta, pg });
    ...
    try {
      ...
      console.warn(
        `[local-db] ⚠️ Branch system may not work correctly for DB ${id} — failed to ensure main branch:`,
        err
      );
    }
  ...
  async start(id: string): Promise<void> {
    // Already running?
    if (this.runningInstances.has(id) || this.sqliteHandles.has(id)) {
      return;
    }
    ...
    if (!meta) {
      throw new Error(`Local database ${id} not found`);
    }
  ...
  private startSqlite(meta: LocalDbMeta): void {
    const id = meta.id;
    const filePath =
      meta.file_path ?? join(getBaseDataDir(), "sqlite", `${id}.db`);
  ...
  async stop(id: string): Promise<void> {
    ...
    if (sqliteHandle) {
      ...
      const filePath =
        sqliteHandle.meta.file_path ??
        join(getBaseDataDir(), "sqlite", `${id}.db`);
      ...
      if (instance) {
        ...
        for (const branch of branches) {
          ...
          try {
            ...
            console.warn(
              `[local-db] Failed to drop branch database "${branch.dbName}" during delete:`,
              err
            );
          }
        }
  ...
  async list(): Promise<LocalDbInfo[]> {
    ...
    return list.map((meta) =>
      this.metaToInfo(
        meta,
        this.runningInstances.has(meta.id) || this.sqliteHandles.has(meta.id)
      )
    );
  }
  ...
  async getStatus(id: string): Promise<LocalDbInfo | null> {
    ...
    return this.metaToInfo(
      meta,
      this.runningInstances.has(id) || this.sqliteHandles.has(id)
    );
  }

  /** Snapshot of currently running local DB instances (sync, in-memory only). */
  getRunningInstancesSnapshot(): Pick<LocalDbInfo, "id" | "name" | "engine">[] {
    const running = new Map<
      string,
      Pick<LocalDbInfo, "id" | "name" | "engine">
    >();
  ...
  private async ensureMainBranch(localDbId: string): Promise<void> {
    const branches = await this.loadBranchList(localDbId);
    if (branches.some((b) => b.isMain)) {
      return;
    }

    const meta = (await this.loadMetaList()).find((m) => m.id === localDbId);
    if (!meta) {
      throw new Error(`Local database ${localDbId} not found`);
    }
  ...
  async createBranch(input: {
    ...
    return this.withLocalDbLock(input.localDbId, async () => {
      ...
      if (meta.engine !== "postgresql") {
        throw new Error(
          "Branching is only supported for PostgreSQL local databases."
        );
      }
      if (hasActiveQueryForConnection(localDbId)) {
        throw new Error(
          "Cannot create branch while queries are running for this connection."
        );
      }

      const branches = await this.reconcileMissingBranchDatabases(
        localDbId,
        meta
      );
      const parentBranchId =
        input.parentBranchId ?? meta.active_branch_id ?? localDbId;
      ...
      try {
        ...
        try {
          ...
          if (dataTables !== undefined && dataTables !== null) {
            const dataTablesSet = new Set(
              dataTables.map((t) => `${t.schema}.${t.table}`)
            );
      ...
      await this.saveBranchList(localDbId, branches);
      return this.branchMetaToInfo(
        newBranch,
        meta,
        meta.active_branch_id === branchId
      );
    });
  }
  ...
  async deleteBranch(localDbId: string, branchId: string): Promise<void> {
    await this.withLocalDbLock(localDbId, async () => {
      ...
      if (hasActiveQueryForConnection(localDbId)) {
        throw new Error(
          "Cannot delete branch while queries are running for this connection."
        );
      }
        ...
        for (const id of [branchId, ...childIds]) {
          ...
          try {
            ...
            try {
              ...
              await client.query(
                `DROP DATABASE IF EXISTS "${target.dbName.replace(/"/g, '""')}"`
              );
            } finally {
              await client.end();
            }
            await closePgResources(
              this.buildBranchConnectionString(meta, target.dbName)
            );
          } catch (err) {
            console.warn(
              `[local-db] Failed to drop branch database "${target.dbName}":`,
              err
            );
          }
        }
      ...
      await this.saveBranchList(
        localDbId,
        branches.filter((b) => !idsToRemove.has(b.id))
      );
    });
  }
  ...
  async switchBranch(localDbId: string, branchId: string): Promise<BranchInfo> {
    return this.withLocalDbLock(localDbId, async () => {
      ...
      const previous = branches.find(
        (b) => b.id === (meta.active_branch_id ?? localDbId)
      );
      const list = await this.loadMetaList();
      const idx = list.findIndex((m) => m.id === localDbId);
      if (idx < 0) {
        throw new Error(`Local database ${localDbId} not found in metadata.`);
      }
      ...
      if (previous) {
        await closePgResources(
          this.buildBranchConnectionString(meta, previous.dbName)
        );
      }
      await closePgResources(
        this.buildBranchConnectionString(meta, branch.dbName)
      );
  ...
  async renameBranch(
    ...
    if (branch.isMain) {
      throw new Error("Cannot rename the main branch.");
    }
  ...
  async getActiveBranchConnectionString(localDbId: string): Promise<string> {
    ...
    if (meta.engine !== "postgresql") {
      throw new Error(
        "Active branch connection string is only available for PostgreSQL local DBs."
      );
    }
    const branches = await this.reconcileMissingBranchDatabases(
      localDbId,
  ...
  async previewDeleteBranch(
    ...
    const branchIds = [
      branchId,
      ...this.collectChildBranchIds(branches, branchId),
    ];
    const toDelete = branchIds
      .map((id) => branches.find((b) => b.id === id))
      .filter((b): b is BranchMeta => Boolean(b))
      .map((b) =>
        this.branchMetaToInfo(
          b,
          meta,
          b.id === (meta.active_branch_id ?? localDbId)
        )
      );
  ...
  async mergeBranchSchema(input: {
    ...
    return this.withLocalDbLock(input.localDbId, async () => {
      const meta = (await this.loadMetaList()).find(
        (m) => m.id === input.localDbId
      );
      if (!meta) {
        throw new Error(`Local database ${input.localDbId} not found`);
      }
      if (meta.engine !== "postgresql") {
        throw new Error(
          "Branch merge is only supported for PostgreSQL local databases."
        );
      }
      if (hasActiveQueryForConnection(input.localDbId)) {
        throw new Error(
          "Cannot merge branch schema while queries are running for this connection."
        );
      }
      ...
      if (!source) {
        throw new Error(`Source branch ${input.sourceBranchId} not found`);
      }
      if (!target) {
        throw new Error(`Target branch ${input.targetBranchId} not found`);
      }
      ...
      const targetSql = new Set(
        targetDdl.scripts.map((s) => normalizeSql(s.sql))
      );
      const statements = sourceDdl.scripts
        .map((s) => s.sql)
        .filter((sql) => !targetSql.has(normalizeSql(sql)));

      if (input.dryRun) {
        return { applied: 0, errors: [], statements };
      }
      ...
      return { applied, errors: result.errors, statements };
    });
  }
  ...
  async hydrate(): Promise<void> {
    ...
    for (const meta of list) {
      ...
      try {
        ...
        console.log(
          `[local-db] Auto-started: ${meta.name} (${meta.id}) [${engine}]`
        );
      } catch (err) {
        console.error(
  ...
  stopAllSync(): void {
    ...
    for (const [id, instance] of this.runningInstances) {
      try {
        ...
        if (
          pgInternal.process &&
          typeof pgInternal.process.kill === "function"
        ) {
