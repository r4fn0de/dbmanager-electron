...
export function parseServerVersion(versionStr: string): ServerVersion {
  ...
  if (!match) {
    ...
    return { isMariaDb, major: 0, minor: 0, patch: 0 };
  }
  return {
    isMariaDb,
...
export function supportsAddColumnIfNotExists(
  ver: ServerVersion,
  dbType: DatabaseType
): boolean {
...
export function supportsCreateIndexIfNotExists(
  ver: ServerVersion,
  dbType: DatabaseType
): boolean {
  if (dbType === "mariadb" || ver.isMariaDb) {
  ...
  const conn = await Promise.race([
    pool.getConnection().then((connection) => {
      if (timedOut) {
        connection.release();
        throw new Error(
          "MySQL connection acquisition timed out (cancelled after timeout)"
        );
      }
      return connection;
    }),
  ...
  const driver: DatabaseDriver = {
    ...
    async dropIndex(connectionString, schema, indexName, _cascade, ifExists) {
      ...
      return withConnection(connectionString, async (conn) => {
        ...
        const tableName = (rows as Array<{ TABLE_NAME: string }>)[0]
          ?.TABLE_NAME;
        if (!tableName) {
          if (ifExists) {
            return buildDropIndexSql(
              dbType,
              schema,
              indexName,
              _cascade ?? false,
    ...
    async executeQuery(connectionString, sql, _signal) {
      return withConnection(connectionString, async (conn) => {
        const [rows, meta] = await conn.query({ rowsAsArray: true, sql });
    ...
    async exportSchemaDdl(connectionString: string) {
      return withConnection(connectionString, async (conn) => {
        ...
        for (const row of pkRows as Array<{
          TABLE_SCHEMA: string;
          TABLE_NAME: string;
          COLUMN_NAME: string;
        }>) {
        ...
        for (const [tableKey, columns] of tableColumns) {
          if (!tableMeta.has(tableKey)) {
            continue; // Views don't have tableMeta entries
          }
          ...
          for (const col of columns) {
            ...
            if (/\bSTORED GENERATED\b|\bVIRTUAL GENERATED\b/i.test(col.EXTRA)) {
              continue;
            }
          ...
          if (pkCols.length > 0) {
            columnDefs.push(
              `  PRIMARY KEY (${pkCols.map((c) => escId(c)).join(", ")})`
            );
          }
          ...
          if (meta?.engine) {
            options.push(`ENGINE=${meta.engine}`);
          }
          if (meta?.autoIncrement) {
            options.push(`AUTO_INCREMENT=${meta.autoIncrement}`);
          }
          if (meta?.collation) {
        ...
        for (const [, fk] of fkMap) {
          ...
          if (fk.deleteRule && fk.deleteRule !== "RESTRICT") {
            fkSql += ` ON DELETE ${fk.deleteRule}`;
          }
          if (fk.updateRule && fk.updateRule !== "RESTRICT") {
            fkSql += ` ON UPDATE ${fk.updateRule}`;
          }
          scripts.push({
    ...
    async getConstraints(connectionString, schema, table) {
      return withConnection(connectionString, async (conn) => {
        try {
          ...
          const msg = err instanceof Error ? err.message : String(err);
          throw new Error(
            `MySQL getConstraints error for ${schema}.${table}: ${msg}`
          );
        }
      });
    },
    ...
    async getDatabaseInfo(connectionString) {
      return withConnection(connectionString, async (conn) => {
        ...
        const version =
          (versionRows as Array<{ version: string }>)[0]?.version ?? "";
        const [encodingRows] = await conn.query(
          "SELECT @@character_set_database AS encoding"
        );
        const encoding =
          (encodingRows as Array<{ encoding: string }>)[0]?.encoding ?? "";
        const [tzRows] = await conn.query(
          "SELECT @@global.time_zone AS timezone"
        );
        const timezone =
          (tzRows as Array<{ timezone: string }>)[0]?.timezone ?? "SYSTEM";
        ...
        try {
          ...
          `);
          const stats = (statsRows as Record<string, string | null>[])[0];
          ...
          activeConnections =
            stats?.active_connections == null
              ? undefined
              : Number(stats.active_connections);
          maxConnections =
            stats?.max_connections == null
              ? undefined
              : Number(stats.max_connections);
          xactCommit =
            stats?.xact_commit == null ? undefined : Number(stats.xact_commit);
          xactRollback =
            stats?.xact_rollback == null
              ? undefined
              : Number(stats.xact_rollback);
          databaseName =
            stats?.database_name == null
              ? undefined
              : String(stats.database_name);
        } catch {
          ...
          try {
            ...
            const uptimeVal = (uptimeRows as Array<{ Value: string }>)[0]
              ?.Value;
            if (uptimeVal != null) {
            ...
            activeConnections =
              (connRows as Array<{ Value: string }>)[0]?.Value == null
                ? undefined
                : Number((connRows as Array<{ Value: string }>)[0].Value);

            const [maxConnRows] = await conn.query(
              "SHOW VARIABLES LIKE 'max_connections'"
            );
            maxConnections =
              (maxConnRows as Array<{ Value: string }>)[0]?.Value == null
                ? undefined
            ...
            xactCommit =
              (commitRows as Array<{ Value: string }>)[0]?.Value == null
                ? undefined
                : Number((commitRows as Array<{ Value: string }>)[0].Value);

            const [rollbackRows] = await conn.query(
              "SHOW STATUS LIKE 'Com_rollback'"
            );
            xactRollback =
              (rollbackRows as Array<{ Value: string }>)[0]?.Value == null
                ? undefined
                : Number((rollbackRows as Array<{ Value: string }>)[0].Value);

            const [dbRows] = await conn.query("SELECT DATABASE() AS db");
            databaseName =
              (dbRows as Array<{ db: string }>)[0]?.db ?? undefined;
          } catch {
        ...
        if (cacheHitRatio == null) {
          try {
            ...
            const innodbStats = innodbRows as Array<{
              Variable_name: string;
              Value: string;
            }>;
            const reads = innodbStats.find(
              (r) => r.Variable_name === "Innodb_buffer_pool_reads"
            )?.Value;
            const requests = innodbStats.find(
              (r) => r.Variable_name === "Innodb_buffer_pool_read_requests"
            )?.Value;
            if (requests != null && reads != null) {
              ...
              if (
                ...
                cacheHitRatio =
                  Math.round(((reqNum - readNum) / reqNum) * 1000) / 10;
              }
            }
          } catch {
    ...
    async getFunctions(connectionString, schema): Promise<SchemaFunction[]> {
      return withConnection(connectionString, async (conn) => {
        try {
          ...
          for (const row of paramRows as Array<{
          ...
          return (
            ...
            name: row.name,
            return_type:
              row.type === "PROCEDURE" ? null : (row.return_type ?? null),
            schema: row.schema,
            type: (row.type === "PROCEDURE" ? "procedure" : "function") as
              | "function"
              | "procedure",
          }));
        } catch (err) {
    ...
    async getIndexes(connectionString, schema, table) {
      return withConnection(connectionString, async (conn) => {
        try {
          ...
          const msg = err instanceof Error ? err.message : String(err);
          throw new Error(
            `MySQL getIndexes error for ${schema}.${table}: ${msg}`
          );
        }
      });
    },
    ...
    async getSchemaSummary(connectionString) {
      return await withConnection(connectionString, async (conn) => {
        ...
        const currentSchema = (dbRows as Array<{ db_name: string | null }>)[0]
          ?.db_name;
        ...
        const parsed = (rawTables as Record<string, unknown>[])
          .map((row) => {
          ...
          .filter(
            (row) =>
              row.name && row.type && row.type.toUpperCase() === "BASE TABLE"
          )
          .map((row) => ({
    ...
    async getTableDetails(connectionString, schema, table) {
      ...
      try {
        ...
        const msg = err instanceof Error ? err.message : String(err);
        throw new Error(
          `MySQL table details error for ${schema}.${table}: ${msg}`
        );
      }
    },
      ...
      return withConnection(connectionString, async (conn) => {
        try {
          ...
          const totalRows = Number.parseInt(
            (countRows as Array<{ cnt: string }>)[0].cnt,
            10
          );
          ...
          const columnStats = await Promise.all(
            (
              ...
              const _isNullable = col.is_nullable === "YES";
              ...
              if (
                ...
                try {
                  ...
                  stat.topValues = (
                    topValuesRows as Array<{ value: unknown; count: string }>
                  ).map((r) => ({
                  ...
                  const uniqueRow = (
                    uniqueRows as Record<string, unknown>[]
                  )[0];
                  if (uniqueRow) {
                    stat.uniqueCount = Number.parseInt(
      ...
      return withConnection(connectionString, async (conn) => {
        try {
          ...
          try {
            ...
            const timeRow = (
              timeRows as Array<{ UPDATE_TIME: string | null }>
            )[0];
          ...
          const msg = err instanceof Error ? err.message : String(err);
          throw new Error(
            `MySQL getTableStats error for ${schema}.${table}: ${msg}`
          );
        }
      });
    },
      ...
      if (dbType === "mariadb") {
        return withConnection(connectionString, async (conn) => {
          ...
          const columnInfo = {
            ...
            defaultExpr: info.COLUMN_DEFAULT,
    ...
    async setColumnDefault(
      ...
      return withConnection(connectionString, async (conn) => {
        ...
        const info = (
          rows as Array<{ COLUMN_TYPE: string; IS_NULLABLE: string }>
        )[0];
        if (!info) {
    ...
    async setColumnNullable(
      ...
      return withConnection(connectionString, async (conn) => {
        ...
        const columnType = (rows as Array<{ COLUMN_TYPE: string }>)[0]
          ?.COLUMN_TYPE;
    ...
    async waitForDatabase(connectionString, maxRetries = 20, intervalMs = 250) {
      ...
      throw new Error(`Database not ready after ${maxRetries * intervalMs}ms`);
    },
  };
