/**
 * Provider Icons - SVG icon components for AI providers
 * Uses official brand SVGs from the icons directory
 */

// OpenAI SVG path data (same for both modes, only fill changes)
const OPENAI_PATH =
...
/**
 * OpenAI Icon - switches fill based on dark mode class
 * Light mode: #101010 (matches OpenAI_light.svg)
 * Dark mode: #fff (matches OpenAI_dark.svg)
 * Icon color is independent of parent text color
 */
export function OpenAiIcon({ className }: IconProps) {
  return (
    ...
    >
      <title>OpenAI</title>
      <path className="openai-icon-path" d={OPENAI_PATH} fill="#101010" />
      <style>{`
        .openai-icon-path { fill: #101010; }
        :global(.dark) .openai-icon-path,
        :global(html.dark) .openai-icon-path { fill: #fff; }
      `}</style>
    </svg>
  );
}
...
export function OpenAICompatibleIcon({ className }: IconProps) {
  return (
    <svg
      className={className}
      fill="none"
  ...
  return (
    ...
    >
      ...
      <path
        ...
        strokeWidth="1.5"
      />
      <circle cx="9.5" cy="13" fill="currentColor" r="1" />
