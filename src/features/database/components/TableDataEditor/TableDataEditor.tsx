...
import {
  buildEffectiveRows,
  getGridCellIndex,
} from "./utils/tableDataTransforms";
...
function TableDataEditorInner(
  ...
  useEffect(() => {
    ...
    const stillExists = effectiveRows.some(
      (entry) => entry.rowKey === expandedRow.rowKey
    );
    if (!stillExists) {
      setExpandedRow(null);
  ...
  const structureStats = useMemo(() => {
    const nullableCount = table.columns.filter(
      (column) => column.is_nullable
    ).length;
    const defaultCount = table.columns.filter(
      (column) => column.column_default !== null
    ).length;
    return {
  ...
  useEffect(() => {
    if (!pendingEditPerfRowKeyRef.current) {
      return;
    }
  ...
  useEffect(() => {
    if (!(externalTableKey && onDirtyChange)) {
      return;
    }
  ...
  useEffect(() => {
    if (disableWindowUnsavedTracking) {
      return;
    }
  ...
  const saveAllDraftsAcrossTabs = useCallback(async () => {
    ...
    for (const [key, draftState] of draftsSnapshot.entries()) {
      ...
      if (dotIdx <= 0 || dotIdx === scopedKey.length - 1) {
        continue;
      }
      ...
      draftsByTableRef.current.set(key, {
        deletes: {},
        inserts: [],
        updates: {},
      });
  ...
  const handleTableKeyDown = useCallback(
    (event: React.KeyboardEvent) => {
      ...
      if (currentRowIndex === undefined || currentColumnIndex === -1) {
        return;
      }
      ...
      if (nextRow && nextColumn) {
        setFocusedCell({ column: nextColumn, rowKey: nextRow.rowKey });
      }
    },
    [
      editingCell,
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-3 border-b bg-background/95 px-3 py-2 backdrop-blur-sm supports-[backdrop-filter]:bg-background/80">
        ...
        <div className="flex min-w-0 items-center gap-1.5">
          {viewMode === "data" && (
            <DropdownMenu>
              <DropdownMenuTrigger
                render={
                  <Button
                    className={cn("gap-1 font-medium", pressableClass)}
                    size="sm"
                    variant="default"
                  >
                    <UiIcon className="h-3.5 w-3.5" name="plus" />
                    Add
                    <UiIcon className="size-3 opacity-50" name="chevron-down" />
                  </Button>
                }
              />
              <DropdownMenuContent align="start" className="min-w-32">
                <DropdownMenuItem onClick={handleAddDraftRecord}>
                  <UiIcon className="size-3.5" name="table" />
                  Row
                </DropdownMenuItem>
                {onRequestAddColumn && (
                  <DropdownMenuItem onClick={onRequestAddColumn}>
                    <UiIcon className="size-3.5" name="columns-3" />
                    Column
                  </DropdownMenuItem>
                )}
              </DropdownMenuContent>
          ...
          <ToggleGroup
            ...
            variant="outline"
          >
            <ToggleGroupItem className="gap-1 px-2 text-[11px]" value="data">
              <UiIcon className="size-3.5" name="table" />
              Data
            </ToggleGroupItem>
            <ToggleGroupItem
            ...
            <ToggleGroupItem className="gap-1 px-2 text-[11px]" value="json">
              <UiIcon className="size-3.5" name="braces" />
              JSON
            </ToggleGroupItem>
          </ToggleGroup>
          ...
          {viewMode === "data" && selectedRowKeys.size > 0 && (
            <Button
              ...
              variant="destructive"
            >
              <UiIcon className="h-3.5 w-3.5" name="trash" />
        ...
        <div className="flex shrink-0 items-center gap-1 text-muted-foreground text-xs">
          ...
          {hasDraftChanges && (
            <span className="flex items-center gap-1">
              {dirtyCounts.inserts > 0 && (
                <span className="text-emerald-600 dark:text-emerald-400">
                  +{dirtyCounts.inserts}
                </span>
              )}
              {dirtyCounts.updates > 0 && (
                <span className="text-amber-600 dark:text-amber-400">
                  ~{dirtyCounts.updates}
                </span>
              )}
              {dirtyCounts.deletes > 0 && (
              ...
              <TooltipTrigger
                render={
                  <Button
                    ...
                    variant="ghost"
                  >
                    <UiIcon className="h-3.5 w-3.5" name="filter" />
                  </Button>
                }
              />
            ...
            <TooltipTrigger
              render={
                <Button
                  ...
                  variant="ghost"
                >
                  <UiIcon className="h-3.5 w-3.5" name="refresh" />
                </Button>
              }
            />
            <TooltipContent side="bottom" sideOffset={4}>
                    ...
                    <DropdownMenuTrigger
                      render={
                        <Button
                          ...
                          variant="ghost"
                        >
                          <UiIcon className="h-3.5 w-3.5" name="columns-3" />
                        </Button>
                      }
                    />
                  }
            ...
            <DropdownMenuContent align="end" className="w-52">
              {onToggleSidebar && (
                <DropdownMenuItem onClick={onToggleSidebar}>
                  <UiIcon
                    className="size-3.5"
                    name={isSidebarVisible ? "panel-left-close" : "panel-left"}
                  />
              ...
              {onExportData && (
                <DropdownMenuItem onClick={onExportData}>
                  <UiIcon className="size-3.5" name="download" />
                  Export data
                </DropdownMenuItem>
              )}
              {onSeedData && (
                <DropdownMenuItem onClick={onSeedData}>
                  <UiIcon className="size-3.5" name="dice" />
                  Seed data
                </DropdownMenuItem>
              )}
              {(onRequestDropColumn ||
                ...
                onRequestSetColumnDefault ||
                onRequestSetColumnNullable) &&
                viewMode === "data" && <DropdownMenuSeparator />}
              {(onRequestDropColumn ||
                onRequestRenameColumn ||
                onRequestAlterColumnType ||
                onRequestSetColumnDefault ||
                ...
                viewMode === "data" && (
                  <DropdownMenuGroup>
                    ...
                    {onRequestRenameColumn && ddlColumnName && (
                      <DropdownMenuItem
                        onClick={() => onRequestRenameColumn(ddlColumnName)}
                      >
                        Rename column
                      </DropdownMenuItem>
                    )}
                    {onRequestAlterColumnType &&
                      ddlColumnName &&
                      columnMap[ddlColumnName] && (
                        <DropdownMenuItem
                          onClick={() =>
                            onRequestAlterColumnType(columnMap[ddlColumnName])
                          }
                        >
                          Alter column type
                        </DropdownMenuItem>
                    ...
                    {onRequestSetColumnDefault &&
                      ...
                      columnMap[ddlColumnName] && (
                        <DropdownMenuItem
                          onClick={() =>
                            onRequestSetColumnDefault(columnMap[ddlColumnName])
                          }
                        >
                          Set / Drop default
                        </DropdownMenuItem>
                    ...
                    {onRequestSetColumnNullable &&
                      ...
                      columnMap[ddlColumnName] && (
                        <DropdownMenuItem
                          onClick={() =>
                            onRequestSetColumnNullable(columnMap[ddlColumnName])
                          }
                        >
                          Set / Drop NOT NULL
                        </DropdownMenuItem>
      ...
      {error && (
        <div className="flex items-center justify-between gap-2 border-b bg-destructive/5 px-3 py-2 text-destructive text-xs">
          ...
          <Button
            ...
            variant="outline"
          >
            <UiIcon className="h-3 w-3" name="refresh" />
            Retry
          </Button>
        </div>
      )}
      ...
      {viewMode === "data" ? (
        ...
        <div className="min-h-0 flex-1 overflow-auto bg-[radial-gradient(circle_at_0%_0%,hsl(var(--muted)/0.32),transparent_44%),radial-gradient(circle_at_100%_100%,hsl(var(--muted)/0.2),transparent_46%)] p-4">
          <div className="mx-auto w-full max-w-5xl rounded-xl border border-border/60 bg-background/92 p-4 shadow-sm backdrop-blur-sm">
            <div className="flex flex-wrap items-center gap-2 border-border/50 border-b pb-4">
              ...
              <div className="ml-auto flex min-w-[190px] items-center gap-2 rounded-md border border-border/60 bg-muted/25 px-3 py-2">
                ...
                <div>
                  ...
                  <p className="font-medium text-foreground text-xs">
                    {structureStats.hasRls ? "Enabled" : "Disabled"}
                  </p>
                </div>
              </div>
            </div>
              ...
              <table className="w-full border-collapse text-xs">
                <thead className="sticky top-0 z-10 bg-background/95 backdrop-blur-sm">
                  <tr className="border-border/50 border-b">
                    <th
                      className="h-8 whitespace-nowrap px-3 text-left font-medium"
                      scope="col"
                    >
                      #
                    </th>
                ...
                <tbody>
                  {table.columns.map((col, i) => {
                    ...
                    return (
                      <tr
                        className="border-border/40 border-b transition-colors hover:bg-muted/30"
                        key={col.name}
                      >
                        <td className="h-9 px-3 align-top font-mono text-[11px] text-muted-foreground">
                          {i + 1}
                        </td>
                        ...
                        <td className="h-9 px-3 pb-2 align-top font-mono leading-6">
                          ...
                          {isPrimaryKey ? (
                            <span className="text-rose-400"> PRIMARY KEY</span>
                          ) : null}
                          {isForeignKey ? (
                            <span className="text-rose-400">
                              ...
                              REFERENCES{" "}
                              {
                                foreignKeysByColumn.get(col.name)
                                  ?.referenced_table
                              }
                              (
                              {
              ...
              <div className="mt-4 rounded-lg border border-border/60 bg-background">
                ...
                <div className="p-2">
                  {primaryKey.map((columnName) => (
                    <div
                      className="rounded-md px-2 py-1.5 font-mono text-xs hover:bg-muted/25"
                      key={`pk-${columnName}`}
                    >
                      <span className="text-rose-400">PRIMARY KEY</span> (
                      {columnName})
                  ...
                  {foreignKeys.map((fk) => (
                    <div
                      ...
                      key={fk.name}
                    >
                      <span className="text-rose-400">FOREIGN KEY</span> (
                      {fk.column_name})
        ...
        <div className="min-h-0 flex-1 overflow-auto bg-[radial-gradient(circle_at_0%_0%,hsl(var(--muted)/0.32),transparent_44%),radial-gradient(circle_at_100%_100%,hsl(var(--muted)/0.2),transparent_46%)] p-4">
          <div className="mx-auto w-full max-w-5xl rounded-xl border border-border/60 bg-background/92 p-4 shadow-sm backdrop-blur-sm">
            <div className="flex flex-wrap items-center justify-between gap-2 border-border/50 border-b pb-3">
              <div>
                <p className="text-[10px] text-muted-foreground uppercase tracking-wide">
                  JSON view
                </p>
            ...
            <div className="mt-3 rounded-lg border border-border/60 bg-[#0f1116]">
              <pre className="max-h-[70vh] select-text overflow-auto whitespace-pre p-4 font-mono text-slate-200 text-xs leading-5">
                {JSON.stringify(
      ...
      <button
        ...
        className="pointer-events-none fixed z-[10000] flex h-6 w-6 -translate-y-1/2 cursor-default items-center justify-center rounded-sm border border-border/70 bg-background text-muted-foreground opacity-0 shadow-xs transition-[background-color,color] duration-150 ease-[cubic-bezier(0.23,1,0.32,1)] hover:bg-muted hover:text-foreground"
        onClick={(event) => {
          event.stopPropagation();
