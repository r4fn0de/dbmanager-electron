...
## Design
Replace the dnd-kit context and sortable wrappers with `Reorder.Group axis="x"` and `Reorder.Item`. Keep the existing tab order callback, container ref, wheel scrolling, close/click handling, and Motion drag feedback. Set `dragMomentum={false}` and a neutral `dragElastic` so the item remains controlled by the horizontal group. Update the `motion` dependency and lockfile to 13.1.0, then validate the reorder smoke test and typecheck.
